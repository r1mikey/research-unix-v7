...
@ i.e. old = sbrk(incr);
@
@ We add the incr arg to the value of __nd, then call the kernel break syscall.
@ On success return, we stash the old __nd, store our new value to __nd and return the old.
@
@ When incr is 0, we do the same thing, just skip the kernel part.
@
@ brk seems worse - just push the value to the kernel, then set it in __nd on success.
@
.global sbrk
.type sbrk,%function
sbrk:
    ...
    ldr     r1, =__nd                                  @ load up the address of __nd (this is the value we want)
    ldr     r1, [r1]                                   @ deref __nd to get the current value
    str     r1, [fp, #-8]                              @ store __nd as a local
    cmp     r0, #0                                     @ is incr 0?
    beq     2f                                         @ yep, incr is 0, skip to the out codepath
    add     r0, r1, r0                                 @ add the new size to the value of __nd to get what we pass to the kernel
...
2:  ldr     r1, [fp, #-4]                              @ load up the original incr
    ldr     r2, [fp, #-8]                              @ load up the original __nd
    add     r0, r2, r1                                 @ we'll save the new break end
    ldr     r1, =__nd                                  @ load up the address of __nd
    str     r0, [r1]                                   @ save the new value of __nd
...
brk:
    ...
    mov     r0, #-1                                    @ syscalls return -1 for error
    b       1f                                         @ jump to the epilogue to return the error to the caller
2:  ldr     r1, =__nd                                  @ load up the address of __nd
    str     r0, [r1]                                   @ store what we passed to the kernel to __nd
